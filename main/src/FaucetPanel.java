...
public class FaucetPanel extends JPanel {
    ...
    private void updateParticlesAndStir() {
        ...
        while (it.hasNext()) {
            Particle p = it.next();
            p.y += p.speed;
            if (p.y >= liquidSurface) it.remove();
        }

        // Start stirring when all particles landed
        if (packet != null && packet.generatedParticles >= packet.totalParticles && particles.isEmpty() && !stirring) {
            stirring = true;
            stirTicks = 0;
            glass.addUnit(packet.type); // only now add liquid
            packet = null;
        }
        ...
        if (!stirring) visualLiquidType = glass.getCurrentLiquid();
    }
    ...
    public void addLiquid(Glass.Liquid type) {
        if (type == Glass.Liquid.WATER) {
            ...
            visualLiquidType = Glass.Liquid.WATER;
            if (glass.isFull()) JOptionPane.showMessageDialog(this, "Warning: Glass is overflowing!", "Overflow", JOptionPane.WARNING_MESSAGE);
        } else {
            if (glass.isEmpty()) {
    ...
    protected void paintComponent(Graphics g) {
        ...
        // Faucet
        g2.setColor(Color.DARK_GRAY); g2.fillRect(NOZZLE_X - 10, NOZZLE_Y, 40, 20);
        g2.setColor(Color.GRAY); g2.fillRect(NOZZLE_X - 5, NOZZLE_Y + 20, 10, 20);

        // Glass outline
        g2.setColor(Color.BLACK); g2.drawRect(CUP_LEFT, CUP_TOP, CUP_WIDTH, CUP_HEIGHT);

        // Sink
        g2.setColor(Color.DARK_GRAY); g2.fillRect(CUP_LEFT - 20, SINK_TOP, CUP_WIDTH + 40, SINK_HEIGHT);
        g2.setColor(Color.BLACK); g2.drawRect(CUP_LEFT - 20, SINK_TOP, CUP_WIDTH + 40, SINK_HEIGHT);

        // Liquid
        g2.setColor(visualLiquidType == Glass.Liquid.WATER ? Color.CYAN :
                visualLiquidType == Glass.Liquid.MILK ? Color.WHITE :
                        new Color(139, 69, 19));
        if (!glass.isEmpty()) g2.fillRect(CUP_LEFT + 1, CUP_TOP + CUP_HEIGHT - liquidHeight, CUP_WIDTH - 1, liquidHeight);

        // Drops
        for (Drop drop : drops) { g2.setColor(Color.BLUE); g2.fillOval(drop.x, drop.y, 8, 8); }
        ...
        // Overflow
        if (glass.isFull()) g2.setColor(Color.BLUE); g2.fillRect(CUP_LEFT, SINK_TOP + 5, CUP_WIDTH, SINK_HEIGHT - 10);
    }
